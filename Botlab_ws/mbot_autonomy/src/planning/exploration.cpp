...
{
    std::lock_guard<std::mutex> autoLock(dataLock_);
    // if (haveNewMap_) std::cout << "at least this works\n";
    // if (haveNewPose_) std::cout << "at least the other one works\n";
...
void Exploration::executeStateMachine(void)
{
    ...
    if(!are_equal(previousPath.path, currentPath_.path) && !returnHome_)
    {
...
bool checkFinished(mbot_lcm_msgs::path2D_t path, mbot_lcm_msgs::pose2D_t pose)
{
    mbot_lcm_msgs::pose2D_t endPoint = path.path.at(path.path_length-1);
    return std::hypot(endPoint.x - pose.x, endPoint.y - pose.y) < 0.1;
}
int8_t Exploration::executeExploringMap(bool initialize)
{
    ...
    */
    ...
    if (!initialize)
    {
        ...
        frontier_processing_t fap = plan_path_to_frontier(frontiers_, currentPose_, currentMap_, planner_);

        // printf("current path length: %ld\n", currentPath_.path.size());
        if (frontiers_.empty()) printf("frontier empty\n");
        if (frontiers_.size() == fap.num_unreachable_frontiers) printf("frontier dead\n");
        


        if (frontiers_.empty() || frontiers_.size() == fap.num_unreachable_frontiers) 
        {
            ctr_frontier_finish++;
            if (ctr_frontier_finish > 3) 
            {
                status.status = mbot_lcm_msgs::exploration_status_t::STATUS_COMPLETE;
                ctr_frontier_finish = 0;
            }
        ...
        {
            currentPath_ = (fap.path_selected.path_length > 1 || currentPath_.path_length <= 1)? fap.path_selected : currentPath_;

            ctr_frontier_finish = 0;
            if (!frontiers_.empty() && currentPath_.path_length <= 1) status.status = mbot_lcm_msgs::exploration_status_t::STATUS_FAILED;
            if (!frontiers_.empty() && currentPath_.path_length > 1) status.status = mbot_lcm_msgs::exploration_status_t::STATUS_IN_PROGRESS;
        }
